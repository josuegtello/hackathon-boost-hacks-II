        ...
        fs.readFile('./data_base/users.json','utf-8',(err,jsonString)=>{  //funcion no bloqueante para leer
            if(err){//lanzar estado de error
              ...
              try {
                ...
                if(!name || !password){ //si algunos de los datos no existe
                  res.status(400);
                  res.send(JSON.stringify({response:"Invalida data format"}));
                  return;
                }
                if(name=="" || password==""){ //si alguno de los datos esta vacio
                  res.status(400);
                  res.send(JSON.stringify({response:"Invalida data format"}));
                  return;
                }
                data.forEach(user => {  //verificamos si el nombre y la contraseña coincide con alguno de nuestra "base de datos"
