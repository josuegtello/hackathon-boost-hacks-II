  ...
  fs.readFile("./data_base/users.json", "utf-8", (err, jsonString) => {
    ...
    if (err) {
      ...
      try {
        ...
        data.forEach((user) => {
          //verificamos si el nombre de usuario o correo ya existen
          if (user.name == name) {
            //tenemos un nombre de usuario igual
            validation = true;
            answer.name = "Username already registered";
          }
          if (user.email == email) {
            //tenemos un email igual
            validation = true;
            answer.email = "Email already registered";
          }
        });
        if (validation == true) {
          //mandamos el estado de error, se esta repitiendo la informacion
